  ...
  div.products
    ...
    transition(name="results-transition" mode="out-in")
      transition-group(tag="div" class="products-list" key="products" v-if="products && products.length" name="list-transition")
        ...
        div.load-more(key="load-more", @click="loadMore", v-show="products.length < allProducts.length") Load more
      div.no-results(key="no-products" v-if="products && !products.length")
